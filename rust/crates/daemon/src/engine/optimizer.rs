...
use audit::{AuditEvent, AuditEventKind};
use intelligence::{IntelligenceConfig, ConsensusReport};
...
impl EvolutionManager {
    ...
    pub fn analyze_performance(&self, config: &IntelligenceConfig) -> Vec<OptimizationProposal> {
        ...
        for event in events {
            if event.kind == AuditEventKind::GovernanceViolation || event.kind == AuditEventKind::SelfRepair {
                if let Some(report_val) = &event.consensus_report {
                    if let Ok(report) = serde_json::from_value::<ConsensusReport>(report_val.clone()) {
    ...
    fn load_recent_events(&self, path: &Path, window: usize) -> Vec<audit::AuditEvent> {
    ...
    fn generate_proposal(&self, event: &audit::AuditEvent, report: &ConsensusReport) -> Option<OptimizationProposal> {
        ...
        let critical_feedback = report.reviews.iter()
            .filter(|r| r.score < 0.5)
            .map(|r| format!("{}: {}", r.judge_name, r.rationale))
            .collect::<Vec<_>>()
            .join("\n");

        if critical_feedback.is_empty() { return None; }
        ...
        Some(OptimizationProposal {
            ...
            rationale: format!("Low consensus score ({:.2}). Critical issues identified by Judges.", report.aggregate_score),
