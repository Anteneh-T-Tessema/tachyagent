    ...
    fn execute(&mut self, tool_name: &str, input: &str) -> Result<String, ToolError> {
        ...
        match tool_name {
            "write_file" | "replace_file_content" | "multi_replace_file_content" => {
                side_effects.push(format!("MUTATION: {} update suppressed", tool_name));
                output = "SUCCESS (Simulated)".to_string();
            }
            "run_command" => {
                side_effects.push("EXECUTION: shell command mocked".to_string());
                output = "Command simulation complete (no side effects)".to_string();
            }
            "delete_file" => {
            ...
            _ => {
                ...
                output = "SUCCESS (Read-only passthrough simulated)".to_string();
            }
        }
...
impl SimulationJudge {
    pub fn analyze(&self, history: &[SimulatedToolCall]) -> (f32, String) {
        ...
        let mutation_count = history.iter().filter(|c| c.side_effects.iter().any(|s| s.contains("MUTATION"))).count();
        let deletion_count = history.iter().filter(|c| c.side_effects.iter().any(|s| s.contains("DELETION"))).count();

        if deletion_count > 0 {
            score -= 0.3 * (deletion_count as f32);
            report.push(format!("WARNING: Detected {} file deletions.", deletion_count));
        }

        if mutation_count > 10 {
            score -= 0.1;
            report.push("CAUTION: Mission involves high volume of mutations (>10 files).".to_string());
        }
