...
use super::{truncate_completion, AgentInfo, ErrorResponse, Response};
...
pub(crate) fn handle_delete_agent(id: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    if id.trim().is_empty() {
        return Response::json(
            400,
            &ErrorResponse {
                error: "agent id required".to_string(),
            },
        );
    }
    let mut s = state
...
pub(crate) fn handle_run_agent(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let prompt = sanitize_prompt(
        &trigger
            .prompt
            .unwrap_or_else(|| "Analyze this event.".to_string()),
        50_000,
    );
    ...
    std::thread::spawn(move || {
        ...
        if let Some(agent) = s.agents.get_mut(&bg_agent_id) {
            ...
            if result.success {
                agent.mark_completed(&stored_summary);
            } else {
                agent.mark_failed(&stored_summary);
            }
        }
        s.publish_event(
            "agent_run_complete",
    ...
    Response::json(
        202,
        serde_json::json!({ "agent_id": agent_id, "status": "running" }),
    )
}

pub(super) async fn handle_complete_stream(
    body: &str,
    ...
    tokio::spawn(async move {
        let (v_backend, err_msg) = {
            ...
            let model_name = req
                .model
                .as_deref()
                .unwrap_or(&s.config.agent_templates[0].model);
            match backend::OllamaBackend::new(
        ...
        if let Some(mut ollama_backend) = v_backend {
            ...
            let _ = ollama_backend
                .send_streaming_generate(backend::OllamaGenerateRequest {
...
pub(super) async fn handle_complete(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let mut ollama =
        match backend::OllamaBackend::new(model_clone, "http://localhost:11434".to_string(), false)
        {
            Ok(b) => b,
            Err(e) => {
                return Response::json(
    ...
    let (gen_result, completion) = tokio::join!(gen_fut, async {
        let mut buf = String::new();
        while let Some(ev) = t_rx.recv().await {
            if let backend::BackendEvent::Text(t) = ev {
                buf.push_str(&t);
            }
        }
        buf
    ...
    let _ = gen_result.as_ref().map(|(_, m)| {
        if let Ok(mut s) = state.lock() {
            s.inference_stats
                .record(m.ttft_ms, m.tokens_per_sec, m.total_tokens);
        }
    });
    ...
    Response::json(
        200,
        serde_json::json!({ "completion": text, "model": model }),
    )
}
...
pub(crate) fn handle_prompt_oneshot(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    if let Some(m) = &req.model {
        template.model = m.clone();
    }
    let session_id = req.session_id.unwrap_or_else(|| {
        format!(
            "cmp-{}",
            template.model.replace(|c: char| !c.is_alphanumeric(), "_")
        )
    });
