...
pub(crate) fn handle_sso_login(state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    Response::Full {
        ...
        body: format!(
            "Redirecting to {}",
            s.identity.sso_manager.build_login_url(Some("/"))
        )
        .into_bytes(),
...
pub(crate) fn handle_sso_callback(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let res = s
        .identity
        .sso_manager
        .process_callback(saml, &mut user_store);
    s.identity.user_store = user_store;
    match res {
        Ok(sess) => Response::json(
            200,
            serde_json::json!({ "token": sess.token, "email": sess.email }),
        ),
...
pub(crate) fn handle_oauth_callback(
    provider_str: &str,
    query: &str,
    ...
    let config = match OAuthClientConfig::from_env(&provider) {
        ...
        None => {
            return Response::json(
                503,
                &ErrorResponse {
                    error: "OAuth2 not configured".to_string(),
                },
            )
        }
    };
    ...
    let result = {
        ...
        s.identity
            .oauth_manager
            .handle_callback(&config, &code, &oauth_state)
    };
    match result {
...
pub(crate) fn handle_oauth_sessions(state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    Response::json(
        200,
        serde_json::json!({ "active_sessions": s.identity.oauth_manager.active_session_count() }),
    )
}
        ...
        Ok(data) => {
            if let Err(e) = license.save(&tachy_dir) {
                return Response::json(
                    500,
                    &ErrorResponse {
                        error: format!("activation succeeded but save failed: {e}"),
                    },
                );
            }
            Response::json(
...
pub(crate) fn handle_usage(state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let users: Vec<serde_json::Value> = counters
        .values()
        .map(|a| {
            serde_json::json!({
                "user_id": a.user_id,
