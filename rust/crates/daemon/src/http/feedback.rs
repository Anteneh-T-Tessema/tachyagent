...
pub(crate) fn handle_list_adapters(state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    Response::json(
        200,
        serde_json::json!({ "count": adapters.len(), "adapters": adapters }),
    )
}
...
pub(crate) fn handle_register_adapter(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    if req.base_model.is_empty() || req.domain.is_empty() {
        return Response::json(
            400,
            &ErrorResponse {
                error: "base_model and domain are required".into(),
            },
        );
    }
    ...
    let id = s
        .adapter_registry
        .register(&req.base_model, &req.domain, &req.adapter_path);
    let adapter = s.adapter_registry.get(&id).cloned();

    s.audit_logger.log(&audit::AuditEvent::new(
        "feedback",
        audit::AuditEventKind::UsageMetering,
        format!(
            "adapter registered: id={id} domain={} model={}",
            req.domain, req.base_model
        ),
    ));
...
pub(crate) fn handle_get_adapter(id: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    match s.adapter_registry.get(id).cloned() {
        None => Response::json(
            404,
            &ErrorResponse {
                error: format!("adapter not found: {id}"),
            },
        ),
...
pub(crate) fn handle_activate_adapter(id: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    let mut s = state
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    if let Err(e) = s.adapter_registry.activate(id) {
        Response::json(404, &ErrorResponse { error: e })
    } else {
...
pub(crate) fn handle_ab_test(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let a = match s.adapter_registry.get(&req.adapter_a).cloned() {
        ...
        None => {
            return Response::json(
                404,
                &ErrorResponse {
                    error: format!("adapter not found: {}", req.adapter_a),
                },
            )
        }
    };
    let b = match s.adapter_registry.get(&req.adapter_b).cloned() {
        ...
        None => {
            return Response::json(
                404,
                &ErrorResponse {
                    error: format!("adapter not found: {}", req.adapter_b),
                },
            )
        }
    };
    ...
    // Auto-update lift score for winner
    {
        ...
        let _ = s
            .adapter_registry
            .update_lift_score(&result.winner, result.lift);
...
pub(crate) fn handle_finetune_trigger(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    if req.base_model.is_empty() {
        return Response::json(
            400,
            &ErrorResponse {
                error: "base_model is required".into(),
            },
        );
    }
    ...
    let dataset_path = workspace_root
        .join(".tachy")
        .join("gold_standard")
        .join("dataset.jsonl");

    match intelligence::FineTuningBridge::trigger_from_dataset(
        &dataset_path,
        &req.base_model,
        &req.adapter_name,
    ) {
        ...
        Ok(job) => {
            ...
            let adapter_id = s
                .adapter_registry
                .register(&req.base_model, &req.adapter_name, "");
...
pub(crate) fn handle_list_finetune_jobs(state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    Response::json(
        200,
        serde_json::json!({ "count": jobs.len(), "jobs": jobs }),
    )
}
