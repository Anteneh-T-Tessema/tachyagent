...
pub(crate) fn handle_search(path_full: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let index = if let Ok(idx) = intelligence::CodebaseIndexer::load_index(ws) { idx } else {
...
pub(crate) fn handle_finetune_extract(body: &str, state: &Arc<Mutex<DaemonState>>, user: &audit::User) -> Response {
    #[derive(Deserialize)]
    struct Req { sessions_dir: Option<String>, gold_standard: Option<bool> }
    let req: Req = serde_json::from_str(body).unwrap_or(Req { sessions_dir: None, gold_standard: None });
    let s = state.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let workspace_root = s.workspace_root.clone();
    let sessions_dir = req.sessions_dir.map_or_else(|| workspace_root.join(".tachy").join("sessions"), std::path::PathBuf::from);
...
pub(crate) fn handle_index_build(_body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    let ws = state.lock().unwrap_or_else(std::sync::PoisonError::into_inner).workspace_root.clone();
...
pub(crate) fn handle_policy_upload(body: &str, state: &Arc<Mutex<DaemonState>>, user: &audit::User) -> Response {
    #[derive(Deserialize)]
    struct Req { name: String, wasm_base64: String }
    let req: Req = match serde_json::from_str(body) {
        Ok(r) => r,
        Err(e) => return Response::json(400, &ErrorResponse { error: format!("invalid body: {e}") }),
    };
    ...
    let wasm_bytes = match base64::engine::general_purpose::STANDARD.decode(&req.wasm_base64) {
        Ok(b) => b,
        Err(e) => return Response::json(400, &ErrorResponse { error: format!("invalid base64 wasm: {e}") }),
    };
    ...
    if let Err(e) = std::fs::write(&policy_path, wasm_bytes) {
        return Response::json(500, &ErrorResponse { error: format!("failed to write policy: {e}") });
    }
...
pub(crate) fn handle_eval_run(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let dataset = intelligence::FinetuneDataset::from_sessions_isolated(&sessions_dir, true, None, None);
    if dataset.entries.is_empty() {
        return Response::json(400, &ErrorResponse { error: "no gold standard sessions found for evaluation".to_string() });
    }
...
pub(crate) fn handle_train_start(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    #[derive(Deserialize)]
    struct Req { adapter_name: String }
    let req: Req = match serde_json::from_str(body) {
        Ok(r) => r,
        Err(e) => return Response::json(400, &ErrorResponse { error: format!("invalid body: {e}") }),
    };
    ...
    let job_id = format!("job-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs());
    ...
    std::thread::spawn(move || {
        ...
        let mut s = state_clone.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        if let Some(j) = s.inference_stats.active_training_jobs.iter_mut().find(|j| j.id == job_id_clone) {
            j.status = intelligence::finetune::TrainingStatus::Completed;
            j.end_time = Some(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs());
        }
        s.save();
    });
...
pub(crate) fn handle_reweight_guidance(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    #[derive(Deserialize)]
    struct Req { hash: String, reward_score: f32 }
    let req: Req = match serde_json::from_str(body) {
        Ok(r) => r,
        Err(e) => return Response::json(400, &ErrorResponse { error: format!("invalid body: {e}") }),
    };
    ...
    let s = state.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    match s.semantic_cache.reweight_by_hash(&req.hash, req.reward_score) {
        Ok(_) => Response::json(200, serde_json::json!({ "status": "updated", "hash": req.hash, "new_score": req.reward_score })),
...
pub(crate) fn handle_harness_start(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let mut s = state.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let harness_id = format!("harness-{}", timestamp());
    ...
    tokio::spawn(async move {
        ...
        // Step 2: Plan (Research-only project modeling)
        {
            ...
            if let Some(h) = s.harnesses.get_mut(&harness_id_clone) {
                ...
                let plan_markdown = intelligence::SubagentManager::research_plan(&registry, &dummy_step);
        ...
        // Step 3: Act (Authorized Execution)
        {
            ...
            if let Some(h) = s.harnesses.get_mut(&harness_id_clone) {
                ...
                if let Some(sub) = h.active_subagents.values_mut().find(|a| a.agent_type == intelligence::SubagentType::Plan) {
        ...
        {
            let report = intelligence::ConsensusEngine::review_simulation(
                    &registry,
    ...
    Response::json(202, &serde_json::json!({ "id": harness_id, "status": "started" }))
}
