...
mod tests {
    ...
    use audit::{hash_api_key, Role, User};

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn health_endpoint_works() {
        let root = std::env::temp_dir().join(format!("tachy-test-{}", chrono_now_secs()));
        let state = tokio::task::spawn_blocking(move || DaemonState::init(root).expect("init"))
            .await
    ...
    async fn get_policy_returns_200_with_defaults() {
        ...
        let mut state = tokio::task::spawn_blocking(move || DaemonState::init(root).expect("init"))
            .await
            .expect("spawn_blocking");
    ...
    async fn set_policy_invalid_json_returns_400() {
        let root =
            std::env::temp_dir().join(format!("tachy-set-policy-test-{}", chrono_now_secs()));
        let mut state = tokio::task::spawn_blocking(move || DaemonState::init(root).expect("init"))
            .await
            .expect("spawn_blocking");
        let admin_key = "admin-key";
    ...
    async fn cancel_agent_existing_returns_200() {
        ...
        let state = tokio::task::spawn_blocking(move || {
            let mut s = DaemonState::init(root).expect("init");
            let _id = s
                .create_agent("code-reviewer", "cancellable task")
                .expect("create");
            s
        })
        .await
    ...
    async fn event_stream_returns_sse_content_type() {
        ...
        match res {
            ...
            Response::Full { .. } => panic!("expected Stream response for /api/events"),
        }
    }
    ...
    async fn run_template_save_and_list() {
        ...
        let res = handle_request(
            &format!(
                "POST /api/run-templates HTTP/1.1\r\nContent-Length: {}\r\n\r\n{body}",
                body.len()
            ),
            &state,
            &limiter,
    ...
    async fn high_risk_operations_are_gated() {
        ...
        let req =
            format!("GET /api/webhooks HTTP/1.1\r\nAuthorization: Bearer {viewer_key}\r\n\r\n");
        let res = handle_request(&req, &state, &limiter, client_ip).await;
        if let Response::Full { status, body, .. } = res {
            assert_eq!(status, 403);
            let body = String::from_utf8_lossy(&body);
