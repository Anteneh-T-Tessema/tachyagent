...
use super::feedback::{
    handle_ab_test, handle_activate_adapter, handle_finetune_trigger, handle_get_adapter,
    handle_gold_standard_extract, handle_gold_standard_status, handle_list_adapters,
    handle_list_finetune_jobs, handle_register_adapter,
};
use super::governance::{
    handle_add_message, handle_approve_patch, handle_approve_plan, handle_audit_export,
    handle_audit_flush, handle_audit_log, handle_audit_verify, handle_create_conversation,
    handle_create_team, handle_dashboard, handle_delete_conversation, handle_fork_session,
    ...
    handle_list_teams, handle_marketplace_list, handle_metrics, handle_promote,
    handle_schedule_task, handle_set_policy, handle_submit_cloud_job, handle_team_agents,
    handle_team_audit,
};
use super::intel::{
    handle_dependency_graph, handle_diagnostics, handle_eval_run, handle_finetune_extract,
    handle_finetune_modelfile, handle_harness_start, handle_index_build, handle_index_status,
    handle_list_harnesses, handle_monorepo, handle_policy_upload, handle_reweight_guidance,
    handle_search, handle_train_start, handle_train_status,
};
use super::runs::{
    handle_approve_task, handle_cancel_parallel_run, handle_delete_run_template,
    handle_event_stream, handle_get_parallel_run, handle_get_run_conflicts, handle_get_run_cost,
    ...
    handle_parallel_run, handle_replay_run, handle_run_history, handle_run_template,
    handle_save_run_template, handle_start_swarm_run,
};
use super::server::{
...
use super::webhooks::{
    handle_list_webhooks, handle_register_webhook, handle_verify_webhook_signature,
};
use super::workers::{
    handle_deregister_worker, handle_list_workers, handle_register_worker, handle_telemetry_flush,
    handle_telemetry_status, handle_worker_heartbeat,
};
use super::yaya::{
...
pub async fn handle_request(
    ...
    let path_full = path_raw
        .split('?')
        .next()
        .unwrap_or("/")
        .trim_end_matches('/');
    let path = if path_full.is_empty() { "/" } else { path_full };
    let query_str = path_raw
        .find('?')
        .map_or("", |i| &path_raw[i + 1..])
        .to_string();
    ...
    match (method.as_str(), path) {
        ...
        ("POST", "/api/models/pull") => {
            gate_action(state, raw, audit::Action::ManageModels, |_| {
                handle_pull_model(&body, state)
            })
        }
        ...
        ("GET", "/api/audit") => gate_action(state, raw, audit::Action::ViewAudit, |_| {
            handle_audit_log(state)
        }),
        ("GET", "/api/audit/export") => gate_action(state, raw, audit::Action::ViewAudit, |_| {
            handle_audit_export(state)
        }),
        ("GET", "/api/audit/verify") => gate_action(state, raw, audit::Action::ViewAudit, |_| {
            handle_audit_verify(state)
        }),
        ("POST", "/api/audit/flush") => {
            gate_action(state, raw, audit::Action::ManageModels, |_| {
                handle_audit_flush(state)
            })
        }
        ("GET", "/api/metrics") => gate_action(state, raw, audit::Action::ViewAudit, |_| {
        ...
        ("POST", "/api/marketplace/install") => {
            gate_action(state, raw, audit::Action::ManageConfig, |_| {
                handle_install(&body, state)
            })
        }
        ("GET", "/api/parallel/runs") => handle_list_parallel_runs(state, raw),
        ("GET", "/api/runs/history") => handle_run_history(state),
        ("POST", "/api/parallel/runs") => gate_action(state, raw, audit::Action::RunAgent, |_| {
            handle_parallel_run(&body, state, raw)
        }),
        ("GET", "/api/cloud/jobs") => handle_list_cloud_jobs(state),
        ("POST", "/api/cloud/jobs") => {
            gate_action(state, raw, audit::Action::ManageCloudJobs, |_| {
                handle_submit_cloud_job(&body, state)
            })
        }
        ...
        ("POST", "/api/swarm/runs") => gate_action(state, raw, audit::Action::RunAgent, |_| {
            handle_start_swarm_run(&body, state)
        }),
        ...
        ("POST", "/api/agents/run") => gate_action(state, raw, audit::Action::RunAgent, |_| {
            handle_run_agent(&body, state)
        }),
        ("GET", "/api/pending-approvals") => handle_list_pending_approvals(state),
        ("POST", "/api/approve") => {
            gate_action(state, raw, audit::Action::ManageGovernance, |_| {
                handle_approve_patch(&body, state)
            })
        }
        ...
        ("POST", "/api/auth/sso/config") => {
            gate_action(state, raw, audit::Action::ManageEnterpriseSSO, |_| {
                handle_sso_config(&body, state)
            })
        }
        ("GET", "/api/search") => handle_search(path_full, state),
        ("GET", "/api/yaya/experts") => handle_yaya_list_experts(&query_str, raw, state).await,
        ("GET", "/api/yaya/retrieval-preferences") => {
            handle_yaya_get_retrieval_preferences(&query_str, raw, state).await
        }
        ("POST", "/api/yaya/retrieval-preferences") => {
            handle_yaya_set_retrieval_preferences(&body, raw, state).await
        }
        ("POST", "/api/yaya/chat") => handle_yaya_chat(&body, raw, state).await,
        ("POST", "/api/yaya/training/examples") => {
            handle_yaya_submit_training_example(&body, raw, state).await
        }
        ("GET", "/api/policy") => gate_action(state, raw, audit::Action::ManageGovernance, |_| {
            handle_get_policy(state)
        }),
        ("POST", "/api/policy") => gate_action(state, raw, audit::Action::ManageGovernance, |_| {
            handle_set_policy(&body, state)
        }),
        ...
        ("GET", "/api/webhooks") => gate_action(state, raw, audit::Action::ManageWebhooks, |_| {
            handle_list_webhooks(state)
        }),
        ("POST", "/api/webhooks") => gate_action(state, raw, audit::Action::ManageWebhooks, |_| {
            handle_register_webhook(&body, state)
        }),
        ("POST", "/api/webhooks/verify") => {
            gate_action(state, raw, audit::Action::ManageWebhooks, |_| {
                handle_verify_webhook_signature(&body, raw, state)
            })
        }
        ("POST", "/api/tasks/schedule") => {
            gate_action(state, raw, audit::Action::ScheduleTask, |_| {
                handle_schedule_task(&body, state)
            })
        }
        ("POST", "/api/license/activate") => {
            gate_action(state, raw, audit::Action::ManageGovernance, |_| {
                handle_license_activate(&body, state)
            })
        }
        ...
        ("POST", "/api/finetune/extract") => {
            gate_action(state, raw, audit::Action::ManageIntelligence, |user| {
                handle_finetune_extract(&body, state, user)
            })
        }
        ...
        ("POST", "/api/gold-standard/extract") => {
            gate_action(state, raw, audit::Action::ManageIntelligence, |_| {
                handle_gold_standard_extract(state)
            })
        }
        // Phase 2: Adapter Registry
        ("GET", "/api/adapters") => handle_list_adapters(state),
        ("POST", "/api/adapters") => gate_action(state, raw, audit::Action::ManageModels, |_| {
            handle_register_adapter(&body, state)
        }),
        ("POST", "/api/adapters/ab-test") => {
            gate_action(state, raw, audit::Action::ManageModels, |_| {
                handle_ab_test(&body, state)
            })
        }
        // Phase 2: Fine-tuning Bridge
        ("GET", "/api/finetune/jobs") => handle_list_finetune_jobs(state),
        ("POST", "/api/finetune/trigger") => {
            gate_action(state, raw, audit::Action::ManageIntelligence, |_| {
                handle_finetune_trigger(&body, state)
            })
        }
        ...
        ("POST", "/api/governance/policy/upload") => {
            gate_action(state, raw, audit::Action::ManagePolicies, |user| {
                handle_policy_upload(&body, state, user)
            })
        }
