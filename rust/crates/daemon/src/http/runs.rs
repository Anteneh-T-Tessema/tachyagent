...
use super::{chrono_now_secs, ErrorResponse, Response};
...
pub(crate) fn handle_list_parallel_runs(state: &Arc<Mutex<DaemonState>>, raw: &str) -> Response {
    let caller_team = super::utils::extract_user(state, raw).and_then(|u| u.active_team_id);
    let s = state
        .lock()
    ...
    let orch = s
        ...
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    let runs: Vec<serde_json::Value> = orch
        .list_runs()
        .iter()
...
pub(crate) fn handle_parallel_run(
    body: &str,
        ...
        match s.identity.quota_store.check_quota(&user.id, role, 0, 0.0) {
            audit::QuotaResult::Exceeded { reason } => {
                return Response::json(
                    429,
                    &ErrorResponse {
                        error: format!("quota exceeded: {reason}"),
                    },
                )
            }
    ...
    std::thread::spawn(move || {
        ...
        if let Ok(mut s) = bg_state.lock() {
            ...
            let (tokens_in, tokens_out, cost) = completed
                .tasks
                .iter()
                .filter_map(|t| t.result.as_ref())
                .fold((0u64, 0u64, 0.0f64), |(ti, to, c), r| {
                    (
                        ti + r.tokens_in as u64,
                        to + r.tokens_out as u64,
                        c + r.cost_usd as f64,
                    )
                });
            ...
            s.identity
                .quota_store
                .record_usage(&quota_user_id, tokens_in + tokens_out, cost);
...
pub(crate) fn handle_get_parallel_run(
    run_id: &str,
    state: &Arc<Mutex<DaemonState>>,
    raw: &str,
) -> Response {
    ...
    Response::json(
        200,
        serde_json::json!({ "run_id": run_id, "status": "running", "tasks": tasks }),
    )
}

pub(crate) fn handle_cancel_parallel_run(
    run_id: &str,
    _body: &str,
    state: &Arc<Mutex<DaemonState>>,
) -> Response {
...
pub(crate) fn handle_get_run_conflicts(
    run_id: &str,
    state: &Arc<Mutex<DaemonState>>,
    raw: &str,
) -> Response {
...
pub(crate) fn handle_start_swarm_run(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    eprintln!(
        "[swarm] run={run_id} tasks={} planner={:?}",
        plan.tasks.len(),
        plan.planner
    );
    let now = chrono_now_secs();
    let agent_tasks: Vec<AgentTask> = plan
        .tasks
        .iter()
        .map(|t| AgentTask {
...
pub(crate) fn handle_get_run_cost(
    run_id: &str,
    state: &Arc<Mutex<DaemonState>>,
    raw: &str,
) -> Response {
...
pub(crate) fn handle_replay_run(run_id: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    let original = if let Some(r) = run_opt {
        r
    } else {
        let history = parallel::Orchestrator::load_run_history(&workspace_root);
    ...
    let tasks: Vec<AgentTask> = original
        .tasks
        .iter()
        .map(|t| AgentTask {
            id: format!("{}-r", t.id),
...
pub(crate) fn handle_save_run_template(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    if req.tasks.is_empty() {
        return Response::json(
            400,
            &ErrorResponse {
                error: "tasks array must not be empty".into(),
            },
        );
    }
    let template = crate::state::RunTemplate {
        ...
        tasks: req
            .tasks
            .into_iter()
            .map(|t| crate::state::TemplateTask {
                template: t.template,
                prompt: t.prompt,
                model: t.model,
                deps: t.deps,
...
pub(crate) fn handle_run_template(
    name: &str,
    _body: &str,
    state: &Arc<Mutex<DaemonState>>,
    raw: &str,
    ...
    if !template.verify_signature() && template.status != crate::state::TemplateStatus::Approved {
        return Response::json(
            403,
            &ErrorResponse {
                error: "template signature verification failed and status is not 'Approved'"
                    .to_string(),
            },
        );
    }
    ...
    let tasks: Vec<AgentTask> = template
        .tasks
        .iter()
        .enumerate()
        .map(|(i, t)| {
            ...
            let deps = t
                ...
                .filter_map(|dep_name| {
                    template
                        ...
                        .iter()
                        .position(|x| {
                            x.template == *dep_name || x.prompt.starts_with(dep_name.as_str())
                        })
    ...
    state
        ...
        .publish_event(
            ...
            serde_json::json!({ "template": name, "run_id": run_id }),
        );
...
pub(crate) fn handle_approve_task(task_id: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    ...
    if approved {
        ...
        Response::json(
            200,
            serde_json::json!({ "ok": true, "task_id": task_id, "status": "queued" }),
        )
    } else {
        Response::json(
            404,
