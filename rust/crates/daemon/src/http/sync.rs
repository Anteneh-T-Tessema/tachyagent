...
use platform::sync::{StateSnapshot, SyncManager};
...
pub async fn handle_receive_sync(body: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    let snapshot: StateSnapshot = match serde_json::from_str(body) {
        Ok(s) => s,
        Err(e) => return Response::json(400, &serde_json::json!({ "error": "Invalid snapshot", "detail": e.to_string() })),
    };
    ...
    let sync_manager = match &s.connectivity.sync_manager {
        Some(sm) => sm,
        None => return Response::json(501, &serde_json::json!({ "error": "Sync not configured on this node" })),
    };
    ...
    match sync_manager.restore_snapshot(&snapshot) {
        ...
        Err(e) => Response::json(500, &serde_json::json!({ "error": "Sync failed", "detail": e.to_string() })),
    }
}
...
pub async fn handle_trigger_beam(query_str: &str, state: &Arc<Mutex<DaemonState>>) -> Response {
    let target_url = query_str.split("target=").nth(1).and_then(|s| s.split('&').next()).unwrap_or("");
    if target_url.is_empty() {
        return Response::json(400, &serde_json::json!({ "error": "Missing target parameter" }));
    }
    ...
    let sync_manager = {
        ...
        match &s.connectivity.sync_manager {
            Some(sm) => sm.clone(),
            None => return Response::json(501, &serde_json::json!({ "error": "Sync not configured" })),
        }
    };

    match sync_manager.beam_to(target_url).await {
        Ok(_) => Response::json(200, &serde_json::json!({ "status": "Beam successful" })),
        Err(e) => Response::json(500, &serde_json::json!({ "error": "Beam failed", "detail": e.to_string() })),
    }
}
...
pub fn handle_sync_pulse(state: &Arc<Mutex<DaemonState>>) -> Response {
    let s = state.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let device_id = s.connectivity.sync_manager.as_ref().map(|sm| sm.device_id()).unwrap_or("unknown");
