...
fn auth_user(
    ...
    match audit::check_permission(user.role, action) {
        ...
        AccessResult::Denied { reason } => {
            Err(Response::json(403, &ErrorResponse { error: reason }))
        }
    }
}
...
pub(super) async fn handle_yaya_list_experts(
    ...
    match response {
        Ok(resp) if resp.status().is_success() => {
            match resp.json::<Vec<YayaExpertsResponseItem>>().await {
                Ok(items) => {
                    ...
                    state_guard.audit_logger.log(
                        &AuditEvent::new(
                            ...
                            format!("listed yaya experts for workspace {workspace}"),
                        )
                        .with_user(user.id)
                ...
                Err(err) => Response::json(
                    502,
                    &ErrorResponse {
                        error: format!("invalid yaya response: {err}"),
                    },
                ),
            }
        }
        Ok(resp) => Response::json(
            resp.status().as_u16(),
            &ErrorResponse {
                error: format!("yaya returned {}", resp.status()),
            },
        ),
        Err(err) => Response::json(
            502,
            &ErrorResponse {
                error: format!("failed to reach yaya: {err}"),
            },
        ),
    }
}
...
pub(super) async fn handle_yaya_get_retrieval_preferences(
    ...
    let Some(subject) = subject else {
        return Response::json(
            400,
            &ErrorResponse {
                error: "subject query parameter is required".to_string(),
            },
        );
    };
    ...
    let response = client
        .get(format!(
            ...
            yaya_base_url().trim_end_matches('/')
        ))
        .headers(yaya_headers())
        .query(&[
            ...
            Ok(payload) => {
                ...
                state_guard.audit_logger.log(
                    &AuditEvent::new(
                        "daemon",
                        AuditEventKind::UserMessage,
                        format!("inspected yaya retrieval preferences for {workspace}/{subject}"),
                    )
                    .with_user(user.id)
...

    {
        ...
        state_guard.audit_logger.log(
            &AuditEvent::new(
                ...
                AuditEventKind::UserMessage,
                format!(
                    "yaya consult requested for {}/{}",
                    req.workspace, req.subject
                ),
            )
            ...
            Ok(payload) => {
                ...
                if let Some(preferences) = &payload.retrieval_preferences {
                    if let Some(strategy) =
                        preferences.get("strategy").and_then(|value| value.as_str())
                    {
                        detail.push_str(&format!(" strategy={strategy}"));
                    }
                    if let Some(sources) = preferences
                    ...
                    if let Some(terms) = preferences
                        .get("preferred_source_terms")
                        .and_then(|value| value.as_array())
                    {
                        let terms = terms
                            .iter()
            ...
            Ok(payload) => {
                ...
                state_guard.audit_logger.log(
                    &AuditEvent::new(
                        ...
                        format!(
                            "updated yaya retrieval preferences for {}/{}",
                            req.workspace, req.subject
                        ),
                    )
                    .with_user(user.id)
            ...
            Ok(payload) => {
                ...
                state_guard.audit_logger.log(
                    &AuditEvent::new(
                        "daemon",
                        AuditEventKind::ToolResult,
                        "submitted approved training example to yaya",
                    )
                    .with_user(user.id)
