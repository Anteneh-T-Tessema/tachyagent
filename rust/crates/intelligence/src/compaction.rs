...
use crate::finetune::{FlashCompactor, CompactionLevel};
...
impl CompactManager {
    ...
    pub fn digest(messages: &[serde_json::Value]) -> String {
        ...
        if let Some(logic) = FlashCompactor::distill_to_level(&dummy_session, CompactionLevel::Logic) {
        ...
        if let Some(sequence) = FlashCompactor::distill_to_level(&dummy_session, CompactionLevel::Sequence) {
