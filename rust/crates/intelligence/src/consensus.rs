...
impl ConsensusEngine {
    ...
    pub fn review_repair(
        ...
        let reviews = vec![
            Self::integrity_judge(repair_diff),
            Self::qa_judge(test_output),
            Self::visual_judge_advanced(visual_report),
        ];
    ...
    pub fn review_scaling(
        ...
        let reviews = vec![
            Self::resource_judge(requested_workers, budget_remaining_usd),
            Self::efficiency_judge(current_workers, requested_workers),
        ];
    ...
    fn integrity_judge(diff: &str) -> JudgeReview {
        ...
        if diff.len() > 5000 {
            score = 0.6;
            rationale = "WARNING: Repair diff is unusually large. Risk of unintended side effects."
                .to_string();
        }
        ...
        JudgeReview {
            judge_name: "IntegrityJudge".to_string(),
            score,
            rationale,
            veto,
        }
    ...
    fn visual_judge_basic(_output: &str) -> JudgeReview {
        JudgeReview {
    ...
    fn visual_judge_advanced(report: Option<&VisualReport>) -> JudgeReview {
        ...
        if let Some(r) = report {
            if !r.passed {
                ...
                veto = true;
                rationale = format!(
                    "VETO: Visual regression detected in {}. Repair rejected.",
                    r.screenshot_path
                );
                ...
                if diff.contains("WARNING") {
                    score = 0.7;
                    rationale =
                        "WARNING: Subtle visual shifts detected. Proceed with caution.".to_string();
                }
            }
        }

        JudgeReview {
            judge_name: "VisualJudge".to_string(),
            score,
            rationale,
            veto,
        }
    ...
    fn security_judge(desc: &str) -> JudgeReview {
        ...
        let mut veto = false;
        let mut rationale =
            "Security sentinel analyzed tool chain: No high-risk commands detected.".to_string();
        ...
        JudgeReview {
            judge_name: "SecuritySentinel".to_string(),
            score,
            rationale,
            veto,
        }
    ...
    fn performance_judge(output: &str) -> JudgeReview {
        let mut score = 1.0;
        let mut rationale =
            "Performance architect: No resource pressure detected in simulation.".to_string();

        if output.contains("timeout") {
            score = 0.3;
            rationale =
                "WARNING: Command timed out during simulation. Potential performance bottleneck."
                    .to_string();
        } else if output.contains("killed") || output.contains("OOM") {
            score = 0.1;
            rationale =
                "CRITICAL: Simulation was killed (Out of Memory). Refinement required.".to_string();
        }

        JudgeReview {
            judge_name: "PerformanceArchitect".to_string(),
            score,
            rationale,
            veto: false,
        }
    ...
    fn qa_judge(output: &str) -> JudgeReview {
        ...
        let mut veto = false;
        let mut rationale =
            "QA engineer: Code successfully compiled and all tests passed.".to_string();

        if output.contains("error:") || output.contains("panic!") || output.contains("FAILED") {
            score = 0.0;
            veto = true;
            rationale = "VETO: Functional failures detected. Recursive self-correction triggered."
                .to_string();
        }

        JudgeReview {
            judge_name: "QAEngineer".to_string(),
            score,
            rationale,
            veto,
        }
    ...
    fn resource_judge(requested: usize, budget_remaining: f32) -> JudgeReview {
        ...
        let mut veto = false;
        let mut rationale =
            "Resource judge: Scaling request within budget constraints.".to_string();
        ...
        if budget_remaining < 0.05 && requested > 2 {
            ...
            score = 0.5;
            rationale =
                "WARNING: Low budget remaining. Scaling approved but restricted.".to_string();
        }

        JudgeReview {
            judge_name: "ResourceJudge".to_string(),
            score,
            rationale,
            veto,
        }
    ...
    fn efficiency_judge(current: usize, requested: usize) -> JudgeReview {
        let mut score = 1.0;
        let mut rationale = format!("Efficiency judge: Scaling from {current} to {requested} workers is justified by current load.");

        if requested > current * 4 {
            score = 0.4;
            rationale = "WARNING: Aggressive scaling detected. Risk of redundant task execution."
                .to_string();
        }

        JudgeReview {
            judge_name: "EfficiencyJudge".to_string(),
            score,
            rationale,
            veto: false,
        }
