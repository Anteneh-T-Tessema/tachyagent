...
impl SyncManager {
    ...
    pub fn create_snapshot(&self) -> Result<StateSnapshot, SyncError> {
        crate::log_info(&format!("Creating sovereign state snapshot for device: {}", self.device_id));
    ...
    pub fn restore_snapshot(&self, snapshot: &StateSnapshot) -> Result<(), SyncError> {
        crate::log_info(&format!("Restoring sovereign state from snapshot (Timestamp: {})", snapshot.timestamp));
    ...
    pub fn create_full_snapshot(&self) -> Result<StateSnapshot, SyncError> {
        crate::log_info(&format!("Creating full sovereign workspace snapshot for device: {}", self.device_id));
    ...
    fn collect_files(&self, base: &Path, current: &Path, files: &mut std::collections::BTreeMap<String, Vec<u8>>) -> Result<(), SyncError> {
        for entry in std::fs::read_dir(current)? {
            ...
            if path.is_dir() {
                ...
                let rel_path = path.strip_prefix(base).map_err(|_| SyncError::Conflict)?
    ...
    pub async fn beam_to(&self, target_url: &str) -> Result<(), SyncError> {
        ...
        let response = client.post(format!("{}/api/sync/receive", target_url))
            .json(&snapshot)
            .send()
            .await
        ...
        if !response.status().is_success() {
            return Err(SyncError::Io(std::io::Error::new(std::io::ErrorKind::Other, format!("Remote rejected sync: {}", response.status()))));
        }
