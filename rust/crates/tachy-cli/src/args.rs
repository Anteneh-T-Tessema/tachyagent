...
pub(crate) fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    if rest.is_empty() {
        return Ok(CliAction::Repl {
            model: if model_explicit { Some(model) } else { None },
        });
    }
    ...
    match rest[0].as_str() {
        ...
        "yaya-preferences" => {
            ...
            let subject = subject
                .ok_or("usage: yaya-preferences --subject <subject> [--workspace <workspace>]")?;
            Ok(CliAction::YayaPreferences {
                workspace,
                subject,
                set_sources,
                set_terms,
        ...
        "run-agent" => {
            let template = rest
                .get(1)
                .ok_or("usage: run-agent <template> <prompt...>")?
                .clone();
            let prompt = if rest.len() > 2 {
                rest[2..].join(" ")
            } else {
                String::new()
            };
            Ok(CliAction::RunAgent {
                template,
                prompt,
                model,
            })
        ...
        "finetune" => {
            ...
            while i < rest.len() {
                match rest[i].as_str() {
                    ...
                    "--base-model" => {
                        base_model = rest.get(i + 1).cloned();
                        i += 2;
                    }
                    _ => {
                        i += 1;
        ...
        "swarm" => {
            ...
            if subcommand == "register" {
                ...
                while i < rest.len() {
                    match rest[i].as_str() {
                        "--goal" | "-g" => {
                            goal = rest.get(i + 1).cloned();
                            i += 2;
                        }
                        "--model" => {
                            swarm_model = rest.get(i + 1).cloned();
                            i += 2;
                        }
