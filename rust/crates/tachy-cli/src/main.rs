...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if needs_license {
        ...
        if let audit::LicenseStatus::TrialActive { remaining_secs } = &status {
            ...
            if days <= 2 {
                eprintln!(
                    "⚠ Trial expires in {} — https://tachy.dev/pricing",
                    status.display()
                );
            }
        }
    ...
    match action {
        ...
        CliAction::ResumeSession {
            session_path,
            command,
        } => setup::resume_session(&session_path, command),
        ...
        CliAction::YayaPreferences {
            ...
            info::handle_yaya_preferences(
                &workspace,
                &subject,
                set_sources.as_deref(),
                set_terms.as_deref(),
                clear,
            )?;
        }
        CliAction::Serve { addr, workspace } => {
            agent::run_serve(&addr, workspace.as_deref()).await?;
        }
        CliAction::RunAgent {
            template,
            prompt,
            model,
        } => agent::run_agent_cmd(&template, &prompt, &model)?,
        ...
        CliAction::InstallOllama => match setup::install_ollama() {
            Ok(()) => {
        ...
        CliAction::Deploy { profile, region } => {
            agent::run_deploy(profile.as_deref(), region.as_deref())?;
        }
        ...
        CliAction::Pipeline {
            ...
            path,
            dry_run,
        } => pipeline::run_pipeline(&subcommand, &path, dry_run)?,
        ...
        CliAction::ExportAudit { format, output } => {
            analysis::run_export_audit(&format, output.as_deref())?;
        }
        CliAction::Finetune { output, base_model } => {
            analysis::run_finetune(output.as_deref(), base_model.as_deref())?;
        }
        CliAction::Policy { subcommand, file } => {
            analysis::run_policy(&subcommand, file.as_deref())?;
        }
        CliAction::Swarm {
            subcommand,
            goal,
...
mod tests {
    ...
    use crate::pipeline::{topological_sort, PipelineStep};
